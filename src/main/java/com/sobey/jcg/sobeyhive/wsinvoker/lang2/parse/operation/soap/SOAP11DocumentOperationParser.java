...
import com.sobey.jcg.sobeyhive.wsinvoker.lang2.parse.operation.util.OrderParamUtil;
...
import com.sobey.jcg.sobeyhive.wsinvoker.lang2.parse.vo.JWSDLPrimativeParamTypeFactory;
...
public class SOAP11DocumentOperationParser extends AOperationParser
{
	...
	public JWSDLOperation getJWSDLOperation(Operation operation, Types types, String namespaceURI) throws WSInvokerException 
	{
		...
		if(logger.isDebugEnabled()) logger.debug(new StringBuilder("方法解析, 方法名:").append(operationQname).toString());
		this.operationName = operationQname;
		/*取得入参和出参描述*/
		Input input = operation.getInput();//获取Input
		Output output = operation.getOutput(); //获取Output

		JWSDLOperation jwsdlOp = null;
		try{
			if(logger.isDebugEnabled()) logger.debug(new StringBuilder("paring document/literal operation : ")
					.append(operationQname).append("........").toString());
			jwsdlOp = this.documentLiteralOperationParse(operationQname, input, output, paramOreder);
		}catch(Exception e){
			if(logger.isDebugEnabled()) logger.error(new StringBuilder("方法:").append(operationQname)
                    .append("无法使用,因为:").toString(),e);
			throw new WSInvokerException(new StringBuilder("方法:").append(operationQname)
					.append("无法使用,因为").append(e).toString());
		}
		if(jwsdlOp!=null){
			jwsdlOp.setStyle(operation.getStyle().toString());
			jwsdlOp.setAvilable(true);
		}
					...
					Output output, List paramOreder) throws Exception
	{
        ...
        /*2015-06-11 wx 将wrapper的解开操作放在解析这里。否则，如果按照【以前的逻辑】：
	...
	private JWSDLParam[] parseMessage(Message message) throws WSInvokerException
	{
		...
		if(logger.isDebugEnabled()) logger.debug(new StringBuilder("|__").append(operationName)
				.append("[").append(message.getQName()).append("]__").append(parts).toString());
		...
		while(itr.hasNext()){
			...
			if(schemaElement==null){
				throw new WSInvokerException(new StringBuilder("方法").append(operationName)
						.append(" 中的part:").append(elementQNameInSchema)
						.append(" 没有document/literal的schemaelment描述,WSDL错误").toString());
			}
			if(logger.isDebugEnabled()) logger.debug(new StringBuilder("|__").append(operationName)
					.append("[").append(message.getQName()).append("]__find element in schema with the parts name: ")
					.append(elementQNameInSchema).toString());
			if(logger.isDebugEnabled()) logger.debug(new StringBuilder("|__").append(operationName).append("[")
					.append(message.getQName()).append("]__").append(schemaElement.getQName()).toString());
			...
			if(type==null){
				throw new WSInvokerException(
						new StringBuilder("Type not found in schema, element=")
						.append(schemaElement.getQName()).toString());
			}

            //2015-05-06 wx 增加一个解析栈。用于嵌套处理
			...
			if(paramType==null){
				throw new WSInvokerException(new StringBuilder("方法").append(operationName).append(" 中的part:")
						.append(elementQNameInSchema).append(" 无法解析的参数类型描述(").append(paramName)
						.append("),WSDL错误").toString());
			}
			...
			long minOccors, long maxOccurs,  Map<QName, JWSDLParamType> typeStack) throws WSInvokerException
	{
        ...
        String unName = new StringBuilder("UnNamed").append(System.currentTimeMillis()).toString();
		...
		if(type instanceof XmlSchemaComplexType){
			...
			if(minOccors>=0&&maxOccurs>1){
                ...
                paramType.setJavaType(Object[].class);
                paramType.setPrimative(false);
				JWSDLParamType arryType = JWSDLPrimativeParamTypeFactory.getPrimaryType(type.getQName());
                paramType.setArrayElementType(arryType);
                paramType.setQname(type.getQName()==null?new QName("UnNamed",unName)
                    :new QName(type.getQName().getNamespaceURI(),"_Array_"+type.getQName().getLocalPart()));
			}else{
				...
				if(typeName==null){
					/*2015-04-27 wx 有可能是枚举类型.说不定还有更多类型*/
				...
				paramType = JWSDLPrimativeParamTypeFactory.getPrimaryType(typeName);
				if(logger.isDebugEnabled()) logger.debug(new StringBuilder("|__")
					.append(operationName).append("[").append(type.getQName())
					.append(",简单类型]_").append(paramType).toString());
			}
		}
	...
	{
        JWSDLParamType jwsdlParamTypes = null;
		...
		if(logger.isDebugEnabled()) logger.debug(new StringBuilder("|__")
			.append(operationName).append("[").append(complexType.getQName()).append("]__解析复杂元素").toString());
		...
		if(particle==null){
			...
			if(collection.getCount()<1){
                ...
                jwsdlParamTypes.setPrimative(false);
	...
	protected JWSDLParamTypeField parseElementToField(XmlSchemaObject schemaObj,
		...
		if(schemaObj instanceof XmlSchemaElement){
			...
			if(logger.isDebugEnabled()) logger.debug(new StringBuilder("|__")
					.append(operationName).append("[").append(schemaObjFromParsed.getQname())
					.append("]__复杂元素子元素{").append(element.getQName().getNamespaceURI())
					.append("}").append(element.getQName().getLocalPart())
					.append("    min").append(element.getMinOccurs())
					.append("  max:").append(element.getMaxOccurs()).toString());
			...
			if(logger.isDebugEnabled()) logger.debug(new StringBuilder("|__")
				.append(operationName).append("[").append(schemaObjFromParsed.getQname())
				.append("]__复杂元素子元素是一个choice类型").toString());
