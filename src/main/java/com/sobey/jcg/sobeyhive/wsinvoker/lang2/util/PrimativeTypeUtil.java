 ...
 * @author wx
 */
public class PrimativeTypeUtil
{
	...
	{
		if(type.isPrimative()&&clazz.isPrimitive()){
			return type.getQname().getLocalPart().equalsIgnoreCase(clazz.getSimpleName());
		}
		if(type.isPrimative()&&String.class.isAssignableFrom(clazz)){
			return type.getQname().getLocalPart().equalsIgnoreCase(clazz.getSimpleName());
		}
		if(type.isPrimative()&&Number.class.isAssignableFrom(clazz)){
	...
	public static Object createPrimativeObject(Class clazz, String value) throws WSInvokerException
	{
		...
		if(Boolean.class.isAssignableFrom(clazz)||boolean.class.isAssignableFrom(clazz)){
			return (value==null||value.length()<1)?new Boolean(false):new Boolean(value);
		}
		if(byte[].class.isAssignableFrom(clazz)||Byte[].class.isAssignableFrom(clazz)){
		...
		if(Date.class.isAssignableFrom(clazz)){
			Date date = null;
			for(int i=0; i<dateFormatStrArray.length; i++){
				try{
					date = new SimpleDateFormat(dateFormatStrArray[i]).parse(value);
					if(date!=null){
						return date;
					}
				}catch(Exception e){
