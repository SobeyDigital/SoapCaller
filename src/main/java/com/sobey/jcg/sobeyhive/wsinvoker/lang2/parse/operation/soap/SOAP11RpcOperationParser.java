...
public class SOAP11RpcOperationParser extends AOperationParser
{
	...
	public JWSDLOperation getJWSDLOperation(Operation operation, Types types, String namespaceURI) throws WSInvokerException {
		...
		jwsdlOperation.setAvilable(true);
	...
	private JWSDLParam[] parseRPCMessage(Message message) throws WSInvokerException
	{
		...
		.append(operationName).append("[").append(message.getQName()).append("]__").append(parts).toString());
		...
		while(itr.hasNext()){
			...
			if(logger.isDebugEnabled()) logger.debug(new StringBuilder("|__")
			.append(operationName).append("[参数名]__").append(paramName).toString());
	...
	{
		JWSDLParamType paramType = JWSDLPrimativeParamTypeFactory.getPrimaryType(typeName);
		if(paramType!=null){
			if(logger.isDebugEnabled()) logger.debug(new StringBuilder("|__").append(operationName)
					.append("[").append(typeName).append(",简单类型]").toString());
			return paramType;
			...
			if(type instanceof XmlSchemaComplexType){
				...
				if(complexType.getParticle()!=null){
					if(logger.isDebugEnabled()) logger.debug(new StringBuilder("|__")
					.append(operationName).append("[").append(type.getName()).append(",复杂类型]").toString());
					/*对于RPC,这表示它不是数组*/
					...
					if(particle instanceof XmlSchemaSequence){
						...
						for(int i=0; i<collection.getCount(); i++){
							...
							if(logger.isDebugEnabled()) logger.debug(new StringBuilder("|__")
									.append(operationName).append("[").append(complexType.getQName())
				...
				throw new WSInvokerException(new StringBuilder("part引用未知类型的对象")
			...
			if(itr.hasNext()){
				...
				if(attrs==null){
					throw new WSInvokerException(
							new StringBuilder("无法取得RPC的WSDL中数组类型")
							.append(complexType.getName())
							.append("的attribute定义")
							.toString()
					);
				}
				...
				for(int i=0; i<attrs.length; i++){
					if(attrs[i].getNodeName().equalsIgnoreCase("wsdl:arrayType")){
						typeClassName = attrs[i].getValue().replaceAll(".*\\:", "")
														   .replaceAll("\\[\\]", "");
					}else{
						typeClassNameNamespace = attrs[i].getValue();
					}
				}
