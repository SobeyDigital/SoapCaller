package com.sobey.jcg.sobeyhive.wsinvoker.lang2.soapmessage.soap11;
	...
	protected Object processReturnMessageBody(SOAPBody body, 
		...
		if(!isMatch){
			...
			for(int i=0; i<debuglist.size(); i++){
				debugInfo.append("                 |_").append(debuglist.get(i)).append("\n");
			}
		...
		try{
			if(type.isArray()){
                if(type.isComplexTypeArray()){
                    paramName = type.getComplexTypeArrayFieldElementName();
                }
				/*数组的处理,先找到数组元素*/
				...
				if(w3cNodelist!=null&&w3cNodelist.getLength()>0){
                    ...
                    if(elements.size()>0) {
                        Object array = this.treasArrayType(type, clazz, elements);
                        return array;
                    }else{
                        return null;
                    }
				...
				if(w3cNode!=null){
					Object o = this.treasComplexObject(type, clazz, w3cNode);
					return o;
				}else{
					throw new Exception("WS返回内容没有节点:"+paramName);
				}
